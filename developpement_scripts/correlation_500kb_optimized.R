...
i=1
list_positions<-c()
for (file in gsub("[a-z,/,.]","\\1",Sys.glob(paste("temp/predictions_with_names/predictions",chromosome,'_',gene,"_*",sep='')))){
    list_positions[i]<-strsplit(gsub("[a-z,/,.]","\\1",Sys.glob(paste("temp/predictions_with_names/predictions",chromosome,'_',gene,"_*",sep=''))),'_')[[i]][5]
    i<-i+1
}
list_positions<-as.character(format(as.numeric(sort(as.numeric(list_positions))), scientific=F,trim=T))
expression <- read.table(paste("temp/expression/",chromosome,'_',gene,".tsv",sep=''), head=T)

for (position in list_positions){
    predictions <- read.table(paste("temp/predictions_with_names/predictions",chromosome,'_',gene,"_",position,".fa.tsv",sep=''), head=T,row.names=1)
...
write.csv(correlations,file=paste("correlations/correlations_",chromosome,'_',gene,'_',gene_name,".csv",sep=''))
#class(predictions)
#dim(predictions)
#summary(predictions)
