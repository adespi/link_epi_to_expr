...
#TO EXTRACT FA SEQ:
for TF in `more gene_info_some_genes.txt |sed 1d|cut -f1`;do
   ...
   if [ "$chromosome" = '' ] || [ -e 'correlations/correlations_'$chromosome'_'$gene'_'$gene_name'.csv' ]
   then
      continue
   fi
   ...
   for position in temp/kipoi_input/$chromosome'_'$gene_*.fa;do
      ...
      then
         rm $position $position.fai temp/information_patient_seq/list_patient_ordered_by_seq_`basename $position .fa` temp/information_patient_seq/nbr_patient_per_seq_`basename $position .fa`
      fi
   done
   for position in `seq $seq_arg`;do
      a=1
   ...
   Rscript correlation_500kb_optimized.R $chromosome $gene $gene_name
   R_end=`date`
   echo -ne "finished!                \r"
